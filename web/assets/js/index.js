...
async function loadProducts() {
    ...
    try {
        // Fetch product data
        const response = await fetch("LoadProduct");  // Replace with actual API endpoint
        ...
        productList.forEach(item => {
            ...
            // Update product details
            productClone.querySelector("[data-product-link]").href = `product.html?pid=${item.id}`;
            productClone.querySelector("[data-product-image]").src = `product_images/${item.id}/image1.png`; // Adjust the image path as necessary
            productClone.querySelector("[data-product-name]").textContent = item.title;
            productClone.querySelector("[data-product-price]").textContent = new Intl.NumberFormat("en-US", {
                style: "currency",
                currency: "USD",
